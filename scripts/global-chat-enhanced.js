...
class GlobalChatManager {
    ...
    setupUserPresence() {
        ...
        this.currentUserPresenceRef = ref(db, `onlineUsers/${this.currentUser.uid}/${sessionId}`);
        const connectedRef = ref(db, '.info/connected');

        // Monitor connection state for robust presence
        onValue(connectedRef, (snap) => {
            if (snap.val() === true) {
                ...
                // 1. Establish the onDisconnect hook BEFORE writing data
                onDisconnect(this.currentUserPresenceRef).remove().then(() => {
                    // 2. Write the presence data
                    set(this.currentUserPresenceRef, userPresenceData);
                });
            }
        });

        // Listen for online users changes to update UI count
    ...
    showMobileDrawer() {
        if (this.mobileDrawer) {
            ...
            console.log('Drawer opened! (main JS)');
        }
    }
